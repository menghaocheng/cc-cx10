...
**环境2**
（可通过对比本机IP是否在192.168.2网段来判断是否应该用这套环境）
...
```bash
adb connect 192.168.2.19:5555
adb -s 192.168.2.19:5555 shell 'cd /data/local && docker rm -f con4; ./update_image.sh; ./load_cx10_image.sh; ./mb4_build_cx10_con4.sh'
```
...
adb -s 192.168.2.19:5555 shell 'cd /data/local && ./load_cx10_image.sh && ./mb4_build_cx10_con4.sh'
```

**ADB 连接恢复**

`con4` 创建成功后，如果 `adb connect 192.168.2.19:5004` 返回 `Connection refused`，先从宿主确认容器是否已运行和端口是否映射：

```bash
adb -s 192.168.2.19:5555 shell 'docker ps -a | grep con4 || true'
adb -s 192.168.2.19:5555 shell 'docker inspect con4 --format "status={{.State.Status}} running={{.State.Running}} ports={{json .NetworkSettings.Ports}}" 2>&1 || true'
```

如果容器已运行且 5004 已映射到容器 5555，重启容器内 TCP adbd：

```bash
adb -s 192.168.2.19:5555 shell 'docker exec con4 /system/bin/sh -c "/system/bin/setprop service.adb.tcp.port 5555; /system/bin/setprop persist.adb.tcp.port 5555; /system/bin/setprop ctl.restart adbd"'
adb disconnect 192.168.2.19:5004
adb connect 192.168.2.19:5004
```
