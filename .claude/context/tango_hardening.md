...
| :--- | :--- | :--- | :--- | :--- | :--- |
| **0.72.1** | 2026-06-16 | V0.72 第二个 VMP target：在 V0.72.0 基础上把 `platform_check_cpu_part()`（loader.c:1430，MIDR_EL1 读取 + PLATFORM_CPU_PARTS 字符串解析循环）由 `OBFUS_CFF` 改为 `noinline,used`（关闭 OLLVM，VMPacker 单独保护，做法同 V0.71 的 platform_check_isa_features），加入 `VMP_LOADER_TARGETS=platform_check_isa_features,platform_check_cpu_part`，`CUSTOM_VERSION=V0.72.1`。同样走 `docker rm -f con4` 冷启动验证：seg0 grow 至 0x8c268，rx-cave payload=12819B（max=0x9d68，余量充足），`sys.boot_completed=1` 全程，`zygote_secondary` 全程 running，dmesg 全程无 signal 11（含 cas-hal-1-1 本次也未触发）；`verify_mustpass` 复测 **11 PASS / 0 FAIL / 0 SKIP**（首次同样 TC-008 偶发 FAIL，复测通过） | **Current** | TC-008 偶发性同 0.72.0 | **Current Release（loader-VMP 扩展到第二个含循环/字符串解析的平台绑定函数，验证 vmpacker 对分支/循环类目标的冷启动正确性）** |
| **0.72.0** | 2026-06-16 | V0.72 Step 0：在 V0.71.2 基础上重新开启 `VMP_LOADER_ENABLE=1`（`VMP_CORE_ENABLE` 保持 0），即 loader(arm96) 自身的 VMP 脚手架 + platform_check_isa_features 叶子函数 VM 化单独验证冷启动。验证流程：`docker rm -f con4` 重建容器 → docker cp 新六件套 → `docker restart con4` 触发冷启动 → dmesg 清空后观察。结果：`sys.boot_completed=1` 全程保持，`zygote_secondary` 全程 running 无 signal 11，仅 `vendor.cas-hal-1-1` 出现 1 次既有已知 signal 11（与 V0.71.2 一致）；`verify_mustpass` 第二次跑 **11 PASS / 0 FAIL / 0 SKIP**（首次跑 TC-008 因 arm96server 刚重启的时序原因 FAIL，复测通过，判定为偶发非回归） | **Current** | TC-008 存在与 arm96server 重启时序相关的偶发性，非本次改动引入 | **Current Release（V0.71 loader-VMP 脚手架正式转正，VMP_LOADER_ENABLE=1 为默认配置）** |
