...
| **战略转向** | VMP 主攻 loader（弃 VMP_CORE） | 坐实 VMP_CORE（libarm96）崩溃根因=vmpacker 不支持 PIE（libarm96 是 ET_DYN/memfd 随机基址）。VMP_CORE **永久关闭**；改 All-in VMP_LOADER（loader=ET_EXEC 固定基址，不踩 PIE），目标"尽可能多藏 loader 核心逻辑"（硬件绑定/hash 校验/解密导入）。 | **Done (2026-06-17)** | 0.72.2 |
| **修复** | OLLVM 真恢复（曾静默 gcc fallback） | `OBFUSCATE_LOADER=1` 自 V0.69 起一直静默回退 gcc（clang-14 仅在 AOSP prebuilt、不在 PATH；旧 plugin ABI 不匹配 prebuilt clang 会段错误）。修复：`hardening/setup_ollvm_toolchain.sh`（探测 clang→ABI 匹配重编 plugin→验证 OLLVM 真生效）+ build fail-loud（不再静默退 gcc）+ 项目 skill `arm96-ollvm-toolchain`。实测 loader/arm96server 均走 clang+ObfusPass。 | **Done (2026-06-18)** | 0.73.0 |
| **加固** | rx-cave 扩容 | `VMP_LOADER_CAVE_PAGESIZE=0x40000`（loader 链接 max-page-size）把 VMP cave 从 ~27KB(OLLVM 开后) 扩到 ~223KB usable（文件 +~196KB）。 | **Done (2026-06-18)** | 0.73.0 |
| **加固** | VMP_LOADER 批量扩 target（26 个） | 逐批把 26 个 GPR 函数 VM 化（每批冷启动 verify_mustpass）：硬件绑定(platform_check_isa_features/cpu_part/core/android_release)、hash 校验(integrity_check_text/self_text_hash16)、libarm96 exec(exec_core)、反调试全簇、授权见证/过期、v051 fault、解密尺寸校验、进程管理。冷启动 11/11（TC-008 偶发时序，重跑过）。VMP target 用 `noinline,used`（关 OLLVM，VM 为唯一层）。 | **Done (2026-06-18)** | 0.73.5 |
| **后续** | crown jewel 手写拆分 VM 化 | ~~`reconstruct_key`~~ **已于 V0.73.9 VM 化达成**（MRS 修复解锁，见下 0.73.9 行）。仅剩 `decrypt_core_to_memfd`(解密编排) 待办，**多轮尝试均失败，2026-06-27 收敛为：当前 vmpacker 无法可靠 VM 化该函数**。诊断历程：(a) V0.73.9 inline 投毒+VM→**真 SIGILL**（libarm96 启动后崩，logcat 有 sys___translator signal 4）；(b) 一度怀疑根因=解密后 scatter-corrupt 投毒块（V051@3302 + Prong2 MAC-mismatch@3350）的 `&_mac_matched` 出参栈寻址/嵌套 VM 守卫被误译；(c) **V0.73.10 fix①**：把两段投毒抽成 native(OLLVM) helper(`apply_v051_fault_poison`/`apply_prong2_scatter_poison`)、只 VM 化编排主体——**热测 4/4 通过(误导)，但冷启动失败**：32 位 zygote 起不来(`ZygoteProcess: Failed to connect to Zygote through socket zygote_secondary`)、`boot_completed` 永不置 1、无任何崩溃信号(静默)。**结论修正**：根因不主要是投毒（早期"投毒是唯一根因"被热重启时序混淆——热测下 zygote_secondary 已建立，掩盖了问题）；是 **decrypt_core 整体在冷启动高并发下无法被 vmpacker 正确 VM 化**，投毒抽离只把失败签名从 SIGILL 变成静默 socket 失败。**铁律(第3次印证)：热测≠冷启动门槛，必须 `docker restart con4` 冷启动 + `verify_mustpass` 判定**。**修复方向**：①修 vmpacker（这个大型 syscall 编排函数的某模式误译，工程量大、动 VM 工具）；或②只 VM 化其中一小段纯 GPR 无 syscall 切片（价值有限）。ROI 偏低——真 crown jewel(reconstruct_key 密钥派生)已 VM 化，decrypt_core 留 OLLVM 是合理平衡点。**当前发布基线=V0.73.9（27 target，含 reconstruct_key，不含 decrypt_core_to_memfd）**。 | **Deferred (decrypt_core 当前 vmpacker 无法可靠 VM 化)** | 0.73.9 |
| **集成** | step3 crown jewel：reconstruct_key VM 化达成 | 把密钥派生函数 `reconstruct_key`（含 `mrs ID_AA64ISAR0_EL1` + 向量化 16B memcpy）由 `OBFUS_CFF_BCF_SUB` 改 `noinline,used`（普通 codegen，**非 optnone**）并加入 `VMP_LOADER_TARGETS`（27 target）。**MRS 修复(step0)是解锁关键**：V0.73.6 VM 内 MRS 读 ISAR0=0→错密钥→SIGSEGV；现 VM 读真值→密钥正确→libarm96 正常解密运行。冷启动(`docker restart con4`,init etime=18s)：`signal 11`=0、`signal 4(SIGILL)`=0、zygote_secondary running、`verify_mustpass` **11 PASS/0 FAIL**（TC-008 偶发重跑全绿）。**同批试 `decrypt_core_to_memfd` 失败**：build 通过(无 unsupported insn)、libarm96 能解密启动(证明密钥对)，但 libarm96 翻译时 **SIGILL**（vmpacker 误译该大型 syscall 编排函数的某指令组合成运行期非法码，单指令都"支持"故未触发 abort 安全网）；已退回 OLLVM，27 target 不含它。 | **Done (2026-06-26)** | 0.73.9 |
| **集成** | vmp 专项收尾 → step2 换入新 vmpacker | 把 vmp 仓库 tag `vmp-mrs-fix-qinsn-2026-06-19`（step0 MRS 静默读0修复 + step1 128-bit Q 指令覆盖）重建产出的 vmpacker 换入 `res/bin/vmpacker`（sha256=`e0526c46…`），`VMP_CORE_TOOL_SHA256` 同步、`CUSTOM_VERSION=V0.73.8`；**26 target 不变**。回归风险点=MRS 修复使 VM 内 `platform_check_isa_features/cpu_part` 从「读 ISAR0/MIDR=0」变「读真值」。验证：`./make.sh --build cx` 通过(BUILD_RC=0)；热部署 32位 `com.dragon.read` 稳定无 signal 11；**冷启动**(`docker restart con4`，init pid1 etime=25s 确为真冷启动)全局 `signal 11` 计数=0、zygote_secondary running、仅既有 `vendor.cas-hal-1-1` SIGABRT(signal 6, 独立 CAS binder 问题非本改动)；`verify_mustpass` **11 PASS / 0 FAIL / 0 SKIP**(TC-008 首跑偶发 FAIL 重跑全绿)。证明 MRS 由读0变读真值后冷启动不回归。 | **Done (2026-06-26)** | 0.73.8 |
...
| :--- | :--- | :--- | :--- | :--- | :--- |
| **0.72.2** | 2026-06-16 | V0.72 第三个 VMP target = `platform_check` 编排主体。**首次直接把 `platform_check` 设为 target 失败**：vmpacker 报 `translation aborted: 6 unsupported instruction(s)`（42/48），6 条全是 NEON（`MOVI v0.16b` / `STP q0,q0` / `STR q`），来自 `struct sigaction sa`（~150B）`memset` 零初始化被向量化；VM 纯 GPR 无 V 寄存器无法翻译，**拒绝产出**（对比 V0.69 静默产崩溃码，是安全网进步）。`target("general-regs-only")` 与 fortified `memset` always_inline 冲突亦不可行。**解法=拆函数**：新增 `platform_check_core(struct sigaction *old_sa)`（loader.c）承载全部编排（含原位置 sigaction 恢复，语义逐字节保持），`platform_check` 退回 `OBFUS_CFF` 只保留 NEON 结构体初始化 + 委托调用；`old_sa` 经 x0 传入。VMP target 改为 `platform_check_core`（无大结构体→纯 GPR→完整翻译），内部 `bl` 到同样 VM 化的 cpu_part/isa_features（嵌套 VM 重入，plan Step1.5 验证）。`VMP_LOADER_TARGETS=...,platform_check_cpu_part,platform_check_core`，`CUSTOM_VERSION=V0.72.2`，rx-cave payload=13167B（max=0x9bd8，余量约26KB）。冷启动验证（基线镜像烤 V0.69.0 确为 crash-loop 对照）：`sys.boot_completed=1`、zygote/zygote_secondary 40s 全程 running、dmesg **零 signal 11**（比 0.72.0 更干净）；`verify_mustpass` 复测 **11 PASS / 0 FAIL / 0 SKIP**（首次 TC-008 偶发 FAIL） | **Current** | TC-008 偶发性同 0.72.0/0.72.1 | **Current Release（loader-VMP 扩展到 platform_check 编排主体；坐实 vmpacker 真实边界=NEON/V 寄存器，靠拆函数把 NEON 留 native 侧规避；风险点3[inline+复杂度]实测不触发）** |
| **0.72.1** | 2026-06-16 | V0.72 第二个 VMP target：在 V0.72.0 基础上把 `platform_check_cpu_part()`（loader.c:1430，MIDR_EL1 读取 + PLATFORM_CPU_PARTS 字符串解析循环）由 `OBFUS_CFF` 改为 `noinline,used`（关闭 OLLVM，VMPacker 单独保护，做法同 V0.71 的 platform_check_isa_features），加入 `VMP_LOADER_TARGETS=platform_check_isa_features,platform_check_cpu_part`，`CUSTOM_VERSION=V0.72.1`。同样走 `docker rm -f con4` 冷启动验证：seg0 grow 至 0x8c268，rx-cave payload=12819B（max=0x9d68，余量充足），`sys.boot_completed=1` 全程，`zygote_secondary` 全程 running，dmesg 全程无 signal 11（含 cas-hal-1-1 本次也未触发）；`verify_mustpass` 复测 **11 PASS / 0 FAIL / 0 SKIP**（首次同样 TC-008 偶发 FAIL，复测通过） | 已被 0.72.2 取代 | TC-008 偶发性同 0.72.0 | loader-VMP 扩展到第二个含循环/字符串解析的平台绑定函数，验证 vmpacker 对分支/循环类目标的冷启动正确性 |
| **0.72.0** | 2026-06-16 | V0.72 Step 0：在 V0.71.2 基础上重新开启 `VMP_LOADER_ENABLE=1`（`VMP_CORE_ENABLE` 保持 0），即 loader(arm96) 自身的 VMP 脚手架 + platform_check_isa_features 叶子函数 VM 化单独验证冷启动。验证流程：`docker rm -f con4` 重建容器 → docker cp 新六件套 → `docker restart con4` 触发冷启动 → dmesg 清空后观察。结果：`sys.boot_completed=1` 全程保持，`zygote_secondary` 全程 running 无 signal 11，仅 `vendor.cas-hal-1-1` 出现 1 次既有已知 signal 11（与 V0.71.2 一致）；`verify_mustpass` 第二次跑 **11 PASS / 0 FAIL / 0 SKIP**（首次跑 TC-008 因 arm96server 刚重启的时序原因 FAIL，复测通过，判定为偶发非回归） | 已被 0.72.2 取代 | TC-008 存在与 arm96server 重启时序相关的偶发性，非本次改动引入 | V0.71 loader-VMP 脚手架正式转正，VMP_LOADER_ENABLE=1 为默认配置 |
| **0.71.2** | 2026-06-15 | `VMP_LOADER_ENABLE=0` + `VMP_CORE_ENABLE=0`：在 V0.71 loader-VMP 脚手架基础上，同时关闭 V0.70 引入的 libarm96 shadow_guard VMP（见 0.70 已知问题）。冷启动（`docker rm -f con4` 重建容器后首次 boot）验证通过：`sys.boot_completed=1`，dmesg 早期窗口外无新增 `signal 11`，`verify_mustpass` **11 PASS / 0 FAIL / 0 SKIP**（TC-004 PASS） | **Current** | `vendor.cas-hal-1-1` 仍处于 `restarting`（独立的既有 CAS binder 问题，不在 mustpass 范围，见第 9 节 `setThreadPoolConfiguration:-25`） | **Current Release（新冷启动基线）** |
| **0.71.1** | 2026-06-15 | 仅 `VMP_LOADER_ENABLE=0`（VMP_CORE 仍为 1）；用于排除 V0.71 loader-VMP/seg0 扩容导致冷启动崩溃的假设 | **Superseded** | 冷启动仍 crash-loop（`zygote_secondary`/`vendor.cas-hal-1-1` 反复 `received signal 11`），证明 loader-VMP 不是根因 | **Archived（诊断中间版本）** |
| **0.71.0** | 2026-06-14 | Loader(arm96) VMP 脚手架 + 首个叶子函数 `platform_check_isa_features`（`vmp_loader.py`，rx-cave 复用 seg0 尾部零间隙，自指环 Prong5 自洽）；详见 `.claude/context/arm96_loader_vmp_plan.md` | **Implemented but disabled** | 继承 0.70 的冷启动 SIGSEGV crash-loop（见下），TC-004 FAIL；脚手架本身（热重启场景）功能验证通过 | **代码已落地，`VMP_LOADER_ENABLE=0` 等待 V0.72 视情况重新评估** |
| **0.70** | 2026-06-xx | `VMP_CORE_TARGETS=0x1049C0-0x104A90:shadow_guard`：将 V0.69 默认入口窗口 VMP 改为 V0.67 RX island 内的 shadow_guard 段（参见 `[[v069-vmp-regression]]` 内存：V0.69 入口窗口 VMP 导致同样崩溃，0.70 的"修复"是改 target，而非修复 vmpacker 寄存器上下文处理） | **⚠️ Critical Regression（2026-06-15 发现）** | **冷启动（cold boot / `docker rm -f` 重建容器后首次 boot）下 `zygote_secondary` 与 `vendor.cas-hal-1-1`（均为 32 位、走 `binfmt_misc→arm96→libarm96` 翻译的进程）反复 `received signal 11`，`init: updatable process ... exited 4 times before boot completed`，`sys.boot_completed` 永不置 1，TC-004 FAIL。该回归在裸 V0.69.0 镜像基线上同样复现，说明是 V0.69→V0.70 引入、且此前验收均只用热重启 `ctl.restart zygote_secondary`（不走该崩溃路径）从未被发现** | **按"失败回滚"准则标记为不可发布；V0.71.2 通过 `VMP_CORE_ENABLE=0` 规避，未来需在 V0.72+ 修复 vmpacker 后才能重新启用** |
| **0.69** | 2026-06-xx | 引入 `vmpacker`（VMP_CORE，独立仓库 `../../../../vmp`），默认入口窗口 VMP target | **Superseded** | 同 0.70：默认入口窗口 VMP 在冷启动下 crash-loop（见 `[[v069-vmp-regression]]`） | **Archived** |
| **0.49** | 2026-05-03 | 当前配置 `CUSTOM_VERSION=V0.49.0`；新增输入指纹门禁(E4901)、patch 前镜像断言(E4902)、patch 后白名单核验(E4903)、`orig_size` 一致性校验(E4905)；擦除策略 remap 为 `.tango→.arm96`、`tango_prop→arm96_prop`、`tango uid→arm96 uid`，保留 `/dev/tango*` | **Stable** | 设备实装版本需重新部署后对外显示 `V0.49.0` | **Archived** |
