...
| **后续** | crown jewel 手写拆分 VM 化 | `reconstruct_key`(密钥派生)/`decrypt_core_to_memfd`(解密) 含 NEON；V0.73.6 试 `optnone` 去 NEON 整体 VM 化→冷启动崩(vmpacker 误译未优化栈寻址，密钥派生错→SIGSEGV)，已回退。需手写 GPR 拆分（-O1 正常 codegen，vmpacker 已证可靠），属高风险 crypto 改动需评审。**当前发布基线=V0.73.7（26 target，不含这两个）**。 | **Deferred** | 0.73.7 |
