...
| **后续** | crown jewel 手写拆分 VM 化 | ~~`reconstruct_key`~~ **已于 V0.73.9 VM 化达成**（MRS 修复解锁，见下 0.73.9 行）。仅剩 `decrypt_core_to_memfd`(解密编排) 待办：V0.73.9 同试 → libarm96 SIGILL（vmpacker 误译大型 syscall 编排函数，非密钥问题），退回 OLLVM。后续需手写 GPR 拆分/缩小 VM 窗口（只 VM 化其密钥相关片段而非整函数），属高风险 crypto 改动需评审。**当前发布基线=V0.73.9（27 target，含 reconstruct_key，不含 decrypt_core_to_memfd）**。 | **Deferred (仅 decrypt_core)** | 0.73.9 |
