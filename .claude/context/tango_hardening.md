...
| **后续** | crown jewel 手写拆分 VM 化 | ~~`reconstruct_key`~~ **已于 V0.73.9 VM 化达成**（MRS 修复解锁，见下 0.73.9 行）。仅剩 `decrypt_core_to_memfd`(解密编排) 待办。**2026-06-26 诊断坐实根因=解密后的 scatter-corrupt 投毒块（V051 fault @loader.c:3302 + Prong2 MAC-mismatch @loader.c:3350）在 VM 下守卫被误判**：实验把这两段投毒临时 `0 &&` 关掉后，decrypt_core VM 化连跑 3 轮 32 位 app 全部正常存活、零 SIGILL/SIGSEGV；只要恢复投毒就崩（两段全开=libarm96 启动后 SIGILL；仅 Prong2=libarm96 直接不启动，崩法随 codegen 漂移）。**非 crypto/密钥问题**（reconstruct_key VM 化没事，GCM 解密本身也对）——是 vmpacker 对投毒块里 `&_mac_matched` 出参栈寻址/`v051_fault_mode_active()` 嵌套 VM 守卫的特定模式误译，错误触发投毒污染合法明文。**修复方向**：①拆函数把两段投毒留 native(OLLVM)、只 VM 化非投毒主体（但主体多为 syscall 管道，藏的价值低）；或②修 vmpacker 出参栈寻址处理（受益所有 target，工程量大）。投毒不可发布关闭，故诊断后已全部回退。**当前发布基线=V0.73.9（27 target，含 reconstruct_key，不含 decrypt_core_to_memfd）**。 | **Deferred (仅 decrypt_core，根因已坐实=投毒块)** | 0.73.9 |
